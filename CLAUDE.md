...
**fortuna** (`ChallengerBudget`, `fortuna × numeroSfidanti`): è la sola cosa che dà peso a una
caratteristica che nel motore vale un punto percentuale di critico e nient'altro. Lo sconto ha **due
limiti che convivono**, e vince il più stringente: un tetto al **30% del monte dichiarato dalla
stazione** e il pavimento di sette punti per sfidante che il toolkit impone. Sui nove monti del
percorso attuale morde sempre il tetto — il pavimento tornerebbe a mordere solo su una stazione che
dichiarasse meno di dieci punti per sfidante, e resta lì come guardia strutturale. Senza tetto lo
sconto arrivava al 47% del monte alla nona prova e schiacciava gli sfidanti sul pavimento in una prova
su nove. Anche gli **sfidanti hanno una progressione d'equipaggiamento** (`ChallengerEquipment`), a
quattro fasce come il loot: prima nascevano tutti `UNCOMMON` con un pezzo d'armatura per tutte e dieci
le prove, e il loro rating restava piatto mentre quello del protagonista raddoppiava. Il loot non si
...
livello (quattro scaglioni: prove 1-2, 3-5, 6-8, 9-10, di cui gli ultimi due portano oggi pesi
identici, quindi gli scaglioni distinti sono di fatto tre). Il pavimento **non sale a ogni scaglione**:
...
e non ha niente di critico: serve solo a serializzare la cronaca in JSON e vive confinata in
`it.fantasyarena.json.JsonSupport`, che è **l'unico `ObjectMapper` del progetto**. Sta in un package
neutro, fuori da `combat`, perché ne hanno bisogno due lettori che non devono conoscersi: la pagina web
(`combat.io.web.ChronicleJson`, che gli delega) e il log analitico (`combat.io.trace`). Prima stava
dentro `combat.io.web`, che è un pozzo che nessun package del percorso console può importare: lasciarlo
là avrebbe costretto a un secondo mapper, e due mapper configurati separatamente divergono senza che
nessun test se ne accorga. Regola dichiarata: **nessun modulo Jackson aggiuntivo**. In particolare, se nel JSON servisse un `Optional`, la risposta è cambiare il dato, non
...
| `combat` | `TrialPlan`: **il percorso come dato**, dieci `TrialStation` in fila. Ogni stazione porta numero, descrizione, quanti sfidanti e la loro `ChallengerOrigin` (`GENERATED`/`MIRROR`, enum e non booleano: una terza origine deve fermare la compilazione); la **forma** dello scontro non è un campo ma si deriva dal numero di sfidanti, e il monte punti c'è solo per gli sfidanti generati — lo specchio ricalca il protagonista, quindi la sua stazione non lo dichiara. Quel monte è quello dell'**intero schieramento**, non del singolo: `FighterFactory` lo ripartisce. Insieme a `ChallengerBudget` è il punto da toccare per allungare o ribilanciare la pressione del percorso. `ChallengerBudget`: il monte di squadra **calato su questo protagonista** — monte dichiarato, sconto della sua fortuna (`fortuna × numeroSfidanti`, letta dalle caratteristiche **effettive**), monte effettivo, con **due limiti che convivono** e vince il più stringente: il tetto del 30% del monte dichiarato (`MAX_LUCK_DISCOUNT_PERCENT`) e il pavimento di sette punti per sfidante che il toolkit impone. Vive qui e non in `HeroBrain` perché uno sconto sugli avversari non è una **scelta** del protagonista: è la pressione del percorso calata su di lui. `ChallengerEquipment`: la stessa idea per l'**equipaggiamento** degli avversari — quattro fasce (prove 1-2, 3-5, 6-8, 9-10) che dichiarano **due** tabelle di rarità distinte, una per l'arma e una per i pezzi d'armatura, più quanti pezzi indossarne. Funzione pura, nessuna estrazione: la casualità resta in `FighterFactory`, che riceve le tabelle e pesca. È il gemello di `TrialLoot` sul lato avversari, e come quello è indicizzato dal numero della prova invece di essere un campo di `TrialStation`, che sarebbe un dato custodito ricavabile dalla stazione stessa. `oneGradeAbove` vive qui perché serve solo allo specchio della decima prova, la cui arma nasce un gradino sopra il grado che la fascia estrae. `TrialLoot`: la **generosità del percorso** verso il protagonista — i quattro scaglioni di rarità (prove 1-2, 3-5, 6-8, 9-10) da cui si estrae il loot di fine prova. Vive qui e non in `HeroBrain` per la stessa ragione di `ChallengerBudget`: il protagonista non *sceglie* quanto pregiato sia l'oggetto che trova, lo trova e basta — semmai decide se tenerlo, e quella scelta resta al cervello. Come `ChallengerEquipment` è una funzione pura del solo numero della prova, e l'estrazione resta in `FighterFactory`. `Arena`: la progressione. Scandisce le stazioni del percorso e la procedura di fine scontro; non decide niente e non calcola niente, e in più **registra**: `run()` restituisce l'`ArenaChronicle`. La scansione è un ciclo che concatena `RoundReport.andThen` stazione per stazione: resta **pigra**, così gli sfidanti di una prova si generano solo se la precedente è stata vinta e `createMirrorRival` si invoca una volta sola, quando la decima prova viene davvero raggiunta. Ogni prova le restituisce un rapporto (`RoundReport`, record privato: se procedere, la scheda cresciuta e le voci di cronaca accumulate) invece di un `Optional`, e i rapporti si concatenano cortocircuitando alla **caduta** — l'accumulo vive nel rapporto e non in un campo, che resterebbe sporco fra due `run()` sulla stessa istanza. Il segnale di via libera si chiama `continues` e non `passed` di proposito: significa «si gioca la prova successiva», che è vero anche dopo un pareggio, non «si è vinto». `RoundOutcome`: com'è finita una prova (`WON`/`FELL`/`STOOD_WITHOUT_WINNING`), calcolato una volta sola e passato a chi lo deve raccontare. Solo `FELL` chiude la corsa: **il pareggio fa proseguire senza premio**, cioè senza loot e senza i tre punti, e la scheda passa alla prova dopo esattamente com'era. Ne segue che il trionfo si dichiara leggendo l'esito dell'ultima prova giocata, non dal fatto che la catena non si sia interrotta — che ora è vero anche di una corsa arrivata in fondo pareggiando. `MatchRunner`: fa giocare **un singolo scontro**, lo consegna a chi lo deve mostrare e ne **restituisce l'esito** (`CombatResult`/`BattleResult`); il *come* e il *quando* mostrarlo sono di un `MatchPresentation` sostituibile. `SilentArenaRun`: un `Supplier<ArenaChronicle>` che gioca l'intera arena senza stampare e senza attendere, assemblando da zero i collaboratori a ogni `get()` — è l'unico posto che può conoscere sia il mondo della cronaca sia quello di `combat.io`. |
| `combat.hero` | Il protagonista: `Hero` (scheda immutabile che sopravvive ai round: arma, armatura per slot, gioielli per tipo; `character()` resta la scheda **base**, `effectiveCharacter()` è la stessa coi buff dell'equipaggiamento addosso — **dato derivato risolto alla lettura**, mai custodito accanto alle fonti), `EquipmentBonus` (il **punto unico** che somma i buff alle caratteristiche base: lo usano sia `combat.factory`, per assemblare il combattente, sia `combat.chronicle`, per fotografare il protagonista, e nessuno dei due deve dipendere dall'altro), `Characteristics` (package-private: l'**unico punto** in cui un `CharacterResult` si ricostruisce a mano sommando dei delta, perché il toolkit non ha API per far crescere un personaggio esistente — rigenerarlo ripescherebbe razza e nome, cioè creerebbe qualcun altro. La usano `EquipmentBonus` e `HeroBrain.grow`, che restano diversi in ciò che aggregano — buff ripetibili gli uni, punti già aggregati gli altri — e condividono solo la ricostruzione, che è la parte che si romperebbe se `CharacterResult` cambiasse forma), `HeroBrain` (**tutte** le sue scelte, compresi i tre comparatori di cernita; non la rarità estraibile del loot, che è generosità del percorso e vive in `TrialLoot`), `Loot` (l'unico oggetto trovato a fine livello: arma, armatura o gioiello, mai più di uno), `HeroProgress` (resoconto della crescita, dati e non stringhe). |
| `combat.chronicle` | **La partita in forma di dati**, e niente altro: `ArenaChronicle` (ingresso del protagonista, **quante prove prevedeva il percorso** (`plannedTrials`), una voce per ogni prova giocata, conclusione — la lunghezza prevista esiste perché il lettore ha solo il JSON e non può contare le stazioni di una tabella Java: senza di essa dovrebbe dedurla dal numero di voci giocate, che è precisamente lo spoiler da evitare), i `CombatSettings` con cui la corsa è stata giocata — trasportati dal motore così come sono, essendo un record profondamente immutabile, e presenti perché senza i pesi i numeri del log analitico non sono interpretabili: chi studia il bilanciamento non può sapere se un danno di 4 nasce da una taratura o da un'altra —, `TrialChronicle` con `TrialShape` (`BATTLE`/`DUEL`) e i passi nella forma che il motore ha prodotto — `rounds` per la battaglia, `turns` per il duello — `HeroSnapshot` (caratteristiche **base ed effettive**, così nessun lettore ricalcola una regola di gioco per mostrare il contributo dell'equipaggiamento)/`CombatantSnapshot` (le sue caratteristiche sono già quelle in campo: il `Fighter` nasce con le effettive, e l'asimmetria è voluta)/`ItemSnapshot` (con i `CharacteristicBonus` che l'oggetto porta, lista vuota se non ne ha), `ProgressChronicle`, `RunConclusion`, più `ChronicleMapper`, unico punto che traduce `Fighter` e `Hero` nelle loro fotografie. **Nessuna stringa di presentazione**, nessuna annotazione Jackson, nessun import da `combat.io` in nessuna direzione. |
| `combat.io.web` | Il secondo lettore della cronaca: `ArenaWebServer` (`HttpServer` del JDK, **solo loopback**, porta parametrica), `ChronicleHandler` (il fornitore della cronaca arriva **iniettato** come `Supplier` e viene invocato una volta per richiesta: ogni apertura della pagina è una partita nuova, e nessuno stato viaggia fra due richieste), `StaticResourceHandler` (lista **chiusa** di tre nomi letterali), `ChronicleJson` (che delega a `it.fantasyarena.json.JsonSupport`, l'unico `ObjectMapper` del progetto). Le risorse della pagina stanno in `src/main/resources/web/`. |
| `it.fantasyarena.json` | `JsonSupport`: **l'unico `ObjectMapper`**, in un package neutro fuori da `combat` perché ne dipendono due lettori che non devono conoscersi — `combat.io.web` e `combat.io.trace`. Non dipende da niente del gioco: se ti trovi a importarci qualcosa di `combat`, la dipendenza va nell'altro verso. |
| `combat.io.trace` | **Il terzo lettore della cronaca: il log analitico per il bilanciamento**, scritto su file e non fatto per gli umani. Tre responsabilità in tre unità, e il confine è il punto: `ChronicleTraceComposer` traduce la cronaca in righe JSON Lines come **funzione pura** (nessun file, nessun `System.out`, l'istante d'apertura entra da un `Clock` iniettato perché sia l'unica cosa non deterministica e sia pilotabile nei test); `TraceFileSelector` **scegli** fra `fantasy-arena-01/02/03.log` interrogando il filesystem e non scrive; `TraceFileWriter` **scrive** in UTF-8 troncando, e non compone né scegli; `TraceRecorder` li mette insieme in tre righe ed è l'unico che legge `java.io.tmpdir` (la cartella è iniettabile, ed è ciò che rende tutto testabile su una cartella temporanea). `TracedChronicleSupplier` decora il `Supplier<ArenaChronicle>` per la modalità web. I cinque eventi, discriminati da `event`: `RUN_OPENED` (istante, `CombatSettings` completi, protagonista, prove previste), `TRIAL_STARTED` (numero, descrizione, forma, roster, monte punti), `EXCHANGE` (**uno per scambio**, con la telemetria del motore), `TRIAL_ENDED` (esito, vitali finali, progressione), `RUN_CLOSED`. Pozzo come `web`: non importa `render`, `log`, `terminal`, `replay` né `web`, e nessuno di quelli lo importa. |
| `combat.factory` | `FighterFactory`: unico punto di contatto coi generatori del toolkit. Decide chi combatte e con che equipaggiamento, poi delega l'assemblaggio al `FighterAssembler` del motore. Genera anche il protagonista, gli sfidanti di ogni round col monte punti e le due tabelle di rarità che la stazione dichiara (`createChallengers(count, squadCharacteristicPoints, weaponRarityTable, armourRarityTable, armourPieceCount)`) e lo specchio finale, materializza il `Fighter` di ogni round da una `Hero` (`summon`) ed estrae il loot di fine livello (`rollLoot`: tipo a caso, rarità estratta dalla tabella pesata che la fascia del percorso dichiara). `STANDARD_EQUIPMENT_RARITY` è la rarità del **solo protagonista**: gli avversari non la usano più, la loro arriva dalla fascia del percorso. Riceve dati grezzi — tabelle e conteggi — e **non importa `ChallengerEquipment`**: `combat` importa già `combat.factory` per il pavimento del monte punti, e il ciclo fra i due package non va chiuso. È `Arena` che scarta la fascia, esattamente come le passa `budget.squadPoints()` e non il `ChallengerBudget`. |
| `combat.io` | Solo contenitore: la presentazione vive nei sotto-package che seguono, disposti a strati con dipendenze a senso unico (`replay` → `log` → `render`, e sia `replay` sia `log` → `terminal`), più i due pozzi indipendenti `web` e `trace`, che non fanno presentazione ma I/O e non conoscono nessuno degli altri. Nessuna classe sta direttamente qui. |
...
- **`combat.io.trace` è il secondo pozzo, e `web` non deve sapere che esista.** Il server continua a
  ricevere un `Supplier<ArenaChronicle>` e nient'altro: il tracciamento si **compone attorno** al
  fornitore (`TracedChronicleSupplier`), non si inietta dentro il server. Il cablaggio delle due strade
  vive in `UiMode`, che è l'unico punto autorizzato a conoscere sia `web` sia `trace`. In console la
  cronaca che `Arena.run()` restituisce non si scarta più: va al tracciatore.
- **Nel tracciamento le tre responsabilità stanno in tre unità, e non è pedanteria.** Comporre le righe
  è una funzione pura testabile senza filesystem; scegliere il file è un'interrogazione del filesystem
  che non scrive; scrivere è I/O che non decide niente. Se ti trovi a leggere una cartella dentro il
  compositore o a comporre una riga dentro lo scrittore, hai fuso due unità che il progetto tiene
  separate per poterle provare una alla volta.
- **Il log analitico va in `java.io.tmpdir`, si chiama `fantasy-arena-01/02/03.log` e ruota su tre.**
  Un file contiene **una corsa intera** e viene troncato, non appeso. Quale dei tre si usa si deriva
  **dallo stato del filesystem** — primo nome mancante, altrimenti data di modifica più vecchia — e mai
  da uno stato in memoria, perché ogni corsa può essere una JVM diversa. Nessuna variabile d'ambiente,
  nessuna configurazione: la cartella è un parametro d'ingresso solo perché i test la sostituiscano.
- **Le corse concorrenti non si proteggono, ed è una decisione dell'utente, non una dimenticanza.** In
  modalità web due richieste simultanee possono scegliere lo stesso file «più vecchio» e sovrascriversi:
  la corsa persa si accetta. Nessun lock chiuderebbe comunque la finestra fra JVM diverse (console e
  server accesi insieme), e il caso d'uso reale è guardare una partita per volta. Non aggiungere lock né
  scrittura-su-temporaneo-e-rinomina «per sicurezza»: il rischio sta dichiarato nel Javadoc di
  `TraceFileSelector`, ed è lì che deve restare.
- **Un guasto di scrittura del log non fa cadere la partita, ma non si silenzia.** La politica vive in
  **un punto solo**, `TraceFileWriter`: l'`IOException` non si propaga — la corsa prosegue e la risposta
  HTTP non si rompe — e viene riportata su `System.err` con il percorso del file e la catena delle cause.
  Non aggiungere un `try/catch` lungo la catena: nasconderebbe la diagnostica invece di aggiungerne.
...
- **`Arena` non decide, scandisce.** Le scelte del protagonista stanno tutte in `HeroBrain`: se
  tenere l'oggetto trovato, con quale criterio confrontarlo con quello che porta già, dove spendere i
  punti. Se ti trovi a scrivere un `if` di gioco dentro `Arena`, appartiene al cervello. `Arena` gli
  passa il numero della prova, non un criterio. Quanto pregiato può essere il loot di quel livello
  **non** è invece una scelta sua: `Arena` la chiede a `TrialLoot`, come già chiede a
  `ChallengerEquipment` con che armi nascono gli avversari.
- **Le leve di bilanciamento sono quattro, e dichiarate.** `HeroBrain` governa la **crescita** del
  protagonista; `TrialPlan` più `ChallengerBudget` governano il **monte punti** del percorso — quanti
  punti ha lo schieramento e quanto la fortuna gliene toglie; `ChallengerEquipment` governa
  l'**equipaggiamento** degli avversari; `TrialLoot` governa la **generosità del loot** che il
  protagonista trova. Tre delle quattro vivono accanto al percorso e non nel cervello proprio perché
  non sono *scelte* del protagonista: sono la pressione — o la generosità — del percorso calata su di
  lui, e al cervello resta solo cosa farne. Cercare una leva sola porta a metterle nel posto sbagliato.
- **Il monte punti è una leva quasi inerte sui rating; l'equipaggiamento è quella vera.** Misurato:
  dodici punti in più di monte di squadra alla nona prova spostano l'offensivo medio degli sfidanti di
  **un** punto, perché il generatore sparge quei punti su sette caratteristiche e sul rating la forza
  pesa `1,0` mentre l'attacco dell'arma pesa `2,0`. Il monte sposta soprattutto `maxHealth`, cioè
  quanto un avversario **resiste**, non quanto **colpisce**. Chi vuole cambiare la pericolosità degli
  avversari muove `ChallengerEquipment`, non `TrialPlan`.
- **Arma e armatura degli sfidanti hanno due tabelle di rarità distinte, e non è ridondanza.** L'arma
  decide quanto fanno male, i pezzi d'armatura quanto resistono, e il bilanciamento deve muovere le due
  grandezze in direzioni opposte: con una tabella sola, alzarla per renderli pericolosi li rende anche
  immortali, abbassarla per renderli abbattibili li rende innocui. La ragione sta nel `max(1, OFF −
  0,5·DEF)` del motore: contro un protagonista a `DEF` 62 servono più di 31 punti di offensivo per fare
  più di un danno, e sopra i 40 tre avversari lo uccidono in quattro turni. Nove punti di offensivo
  separano l'inoffensivo dal letale. Non unificare le due tabelle «per semplificare».
- **La difesa del protagonista cresce coprendo slot, non trovando oggetti pregiati.**
  `HeroBrain.chooseArmourPiece` prende qualunque pezzo per una parte del corpo ancora scoperta, a
  prescindere dalla rarità. Ne segue che ritoccare le tabelle del loot **non** contiene la sua difesa:
  misurato, rendere il loot molto più avaro ha lasciato la difesa alla nona prova dov'era. Chi vuole
  contenerla deve agire sul numero di slot che il protagonista arriva a coprire.
- **I pesi delle tabelle di bilanciamento si tarano misurando, non a intuito.** Il sistema è molto
  sensibile: due punti e mezzo di rating medio in più sugli sfidanti della quarta prova valgono dieci
  punti percentuali di mortalità. `BalanceProbe` (in `src/test/java`, fuori dalla suite) gioca mille
  corse mute e stampa la distribuzione degli esiti, i rating medi per prova e il danno a colpo:
  `mvn -q test-compile exec:java -Dexec.mainClass=it.fantasyarena.combat.BalanceProbe -Dexec.classpathScope=test`.
  La taratura corrente centra il bersaglio dichiarato dall'utente — chi supera la quarta prova arriva
  in fondo nel 35% dei casi — al 34,9%. Prima di ritoccare un peso, misura; dopo averlo ritoccato,
  rimisura.
...
Le leve del percorso hanno test propri, e vale la pena sapere **cosa** vi si asserisce.
`ChallengerBudgetTest` verifica che i due limiti dello sconto convivano e che vinca il più stringente,
non i valori di un caso particolare. `TrialLootTest` verifica i confini dei quattro scaglioni e il
pavimento estraibile di ciascuno — non i pesi, che sono taratura. `ChallengerEquipmentTest` verifica **invarianti di progetto e non
la taratura corrente**: la fascia 1-2 riproduce identico l'equipaggiamento di partenza, la rarità
massima estraibile e il numero di pezzi non decrescono mai passando alla fascia successiva, la tabella
dell'arma non è mai più prudente di quella dell'armatura, e l'ultima fascia può ancora pescare `EPIC`
sull'arma. È una distinzione che è già costata due test: asserire «il pavimento di rarità sale a ogni
fascia» sembrava un invariante ed era invece una proprietà accidentale dei pesi di quel momento, che la
taratura ha dovuto smentire. Un test che si rompe a ogni ritocco dei pesi sta verificando la taratura,
non il progetto.

`BalanceProbe` non è un test e surefire non la raccoglie: è una **sonda di misura** con un `main` che
gioca mille corse mute e stampa la distribuzione degli esiti. Non ha un verde e un rosso, e mille corse
dentro la suite sarebbero minuti a ogni build. Sta in `src/test/java` perché lì trova la passata muta e
il classpath che le serve. Il piano di lavoro prevedeva di rimuoverla a fine bilanciamento: è stata
tenuta perché i pesi si ritoccano più di una volta e senza sonda si torna a tarare a intuito.
...
test propri come sopra. Ne segue una terza, che è la stessa vista da un'altra parte: i due `launch()` di
`UiMode` non hanno test, quindi **il cablaggio del tracciamento nelle due modalità si verifica per
lettura**. Quel che si testa è il decoratore in isolamento (`TracedChronicleSupplierTest`), non l'avvio.

Sul log analitico (`combat.io.trace`) valgono tre regole. La composizione si prova **senza toccare il
filesystem** — è una funzione pura da cronaca a righe, e ogni riga si verifica **deserializzandola**, non
guardandola. Rotazione e scrittura si provano su una **cartella temporanea** di JUnit, mai su
`java.io.tmpdir` reale, che è la cartella vera dell'utente. E la politica di errore si prova per davvero,
provocando un guasto e verificando che l'eccezione non arrivi al chiamante ma la diagnostica sì: un test
che si limita a scrivere un file riuscito non dice niente sulla parte che conta.
...
- I file `spec-<compito>.md` e `implementation-<compito>.md` sono **documenti di lavoro provvisori**:
  vivono il tempo di realizzare quel compito e vengono cancellati a lavoro concluso. Non vanno mai
  citati da questa sezione — un rimando a un documento che verrà cancellato è un riferimento rotto in
  attesa. Quello che merita di sopravvivere alla loro cancellazione (una decisione di design, una
  trappola, la ragione di un numero) va riscritto nel Javadoc della classe interessata o qui come
  contenuto proprio.
...
Due problemi di bilanciamento restano aperti dal ribilanciamento della progressione, e sono decisioni
dell'utente, non lacune da colmare per iniziativa: la **distribuzione bimodale** degli esiti (metà
delle corse muore alla prima prova, metà arriva in fondo) e l'**entità dello sconto della fortuna**.
Chi ritocca i numeri della progressione non deve peggiorare il primo.
